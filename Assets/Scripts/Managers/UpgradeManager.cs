...
namespace IdleGame.Managers
{
    public class UpgradeManager : MonoBehaviour
    {
        ...
        private void CreateDefaultUpgrades()
        {
            ...
            list.Add(Make("연사 장치",    "자동공격 속도 +0.1회/초 / 레벨 (최대 5레벨)", 3_000,  1.72f,  5, StatType.AutoAttackSpeed,  0.1, unlock: 2));

            // ── 스테이지 3: 클릭·자동 중급 강화 ─────────────────────────
            list.Add(Make("강타 훈련",    "클릭 데미지 +10 / 레벨",                     5_000,  1.65f,  0, StatType.ClickDamage,     10,   unlock: 3));
            list.Add(Make("강화 타격기",  "자동공격 데미지 +4 / 레벨",                   6_000,  1.65f,  0, StatType.AutoDamage,       4,   unlock: 3));

            // ── 스테이지 4: 골드 + 자동공속 강화 ────────────────────────
            list.Add(Make("금 감지",      "골드 배율 +0.1 / 레벨",                      15_000, 1.70f,  0, StatType.GoldMultiplier,  0.1,  unlock: 4));
            list.Add(Make("고속 연사",    "자동공격 속도 +0.2회/초 / 레벨 (최대 6레벨)", 20_000, 1.76f,  6, StatType.AutoAttackSpeed,  0.2, unlock: 4));

            // ── 스테이지 5: 고급 공격력 ──────────────────────────────────
            list.Add(Make("일격필살",     "클릭 데미지 +20 / 레벨 (최대 12레벨)",        40_000, 1.78f, 12, StatType.ClickDamage,     20,   unlock: 5));
